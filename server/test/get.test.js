...
describe('GET /api/v1/search', () => {
    it('should return a 200 status code and valid response', async () => {
        const res = await chai
            .request(app)
            .get('/api/v1/search')
            .query({ page: 1, q: 'naruto', categories: 'general,anime', sort: 'views' });
        ...
        expect(res.body.queryDetail.categoryArrToLowerCase).to.deep.equal(['general', 'anime']);
...
describe('GET /api/v1/wallpaper/{imageEndpoint}', () => {
    it('should return a 200 status code and valid response', async () => {
        ...
        const res = await chai.request(app).get(`/api/v1/wallpaper/${imageEndpoint}`);
