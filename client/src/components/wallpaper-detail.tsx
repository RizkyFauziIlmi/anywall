...
import { ChevronLeft, DownloadCloud, FolderCheckIcon, Loader2, Presentation, Trash, Wallpaper } from "lucide-react";
import { Button } from "./ui/button";
import { writeBinaryFile, BaseDirectory, exists, removeFile } from "@tauri-apps/api/fs";
...
export default function WallpaperDetail() {
  const [data, setData] = useState<ApiResponseWallpaper | null>(null);
  const bottomRef = useRef<null | HTMLDivElement>(null)
  const [isLoading, setIsLoading] = useState<string | null>(null);
  const [isDownloaded, setIsDownloaded] = useState<boolean | null>(null);
  const [isImageFullscreen, setIsImageFullscreen] = useState<boolean | undefined>(undefined);
  ...
  const downloadPhoto = async () => {
    ...
    try {
      ...
      if (isExist) {
        ...
        if (permissionGranted) {
          sendNotification({ body: `${fileName} downloaded`, title: 'Success' });
        }
  ...
  const removePhoto = async () => {
    ...
    const confirmed = await confirm(`${fileName} will removed from your device, are you sure?`, { title: "Delete photo", type: "info" })
    ...
    const getInitData = async () => {
      try {
        ...
        if (response.ok) {
          ...
          setData(response.data as ApiResponseWallpaper);
        } else {
  ...
  return (
    <AppLayout isFullscreen={isImageFullscreen}>
      <div className="relative flex flex-col justify-center items-center py-2 min-h-screen px-5">
        <Breadcrumb />
        <img
          src={data.data.imageUrl}
          className={cn("rounded-sm w-3/4", isImageFullscreen ? "absolute top-0 left-0 right-0 bottom-0 w-auto" : "")}
          alt={data.data.name}
        />
        <p className="font-semibold text-sm text-center pt-2">{data.data.name}</p>
        <div className={cn("flex w-full gap-2 justify-center items-center pt-4")}>
          ...
          >
            {isDownloading ? <Loader2 className="animate-spin h-4 w-4 mr-2" /> : isDownloaded ? <FolderCheckIcon className="h-4 w-4 mr-2" /> : <DownloadCloud className="h-4 w-4 mr-2" />}
            {isDownloading ? "Downloading..." : isDownloaded ? "Downloaded" : "Download"}
          </Button>
          <Button onClick={setAsWallpaper} disabled={isSettingWallpaper} variant={"ghost"}>
            {isSettingWallpaper ? <Loader2 className="animate-spin h-4 w-4 mr-2" /> : <Wallpaper className="h-4 w-4 mr-2" />}
            {isSettingWallpaper ? "setting..." : "Set as Wallpaper"}
          </Button>
          <Button onClick={setPreview} variant={"ghost"} >
            <Presentation className="h-4 w-4 mr-2"/>Preview
          </Button>
          {isDownloaded && (
            <Button disabled={isRemoving}  variant={"destructive"} onClick={removePhoto}>
              {isRemoving ? <Loader2 className="animate-spin h-4 w-4 mr-2" /> :  <Trash className="h-4 w-4 mr-2"/>}
              {isRemoving ? "removing..." : "Remove File"}
            </Button>
          )}
        </div>
        <div className={cn("absolute bottom-2 left-2 z-50 font-semibold text-orange-300", isImageFullscreen ? "" : "hidden")}>Press Escape to Exit Fullscreen</div>
      </div>
      <div ref={bottomRef}/>
    </AppLayout>
  );
